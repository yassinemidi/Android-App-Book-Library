...
import com.example.milolibrairy.databinding.FragmentMyBooksBinding
 ...
 * Use the [MyBooks.newInstance] factory method to
...
class MyBooks : Fragment() {
    ...
    override fun onCreateView(
        ...
        for (res:reservation in myres){
            ManageBook(requireContext()).findbook(res.id_book)?.let { tab.add(it) }
        ...
        var adapter= RvMyBookAdapter(tab,requireContext(),activity as IndexActivity)
        binding.rvmybooks.layoutManager= LinearLayoutManager(this.activity,LinearLayoutManager.HORIZONTAL,false)
